...
class browser extends uploader {
	...
	protected function act_upload() {
        ...
        if ($info['extension'] == "zip"){ 
             //2014-02-18 wjh  解压缩功能
            $zip = new ZipArchive() ;
            if ($zip->open($dir."/".$this->file['name']) !== TRUE) { 
                $this->errorMsg('不能打开压缩文件');
            }
            $res = $zip->extractTo($dir); 
            for($i = 0; $i < $zip->numFiles; $i++) {
                $filename = $zip->getNameIndex($i);
                $fileinfo = pathinfo($filename);
                $fileinfos[] = $dir.'/'.$fileinfo['basename'];
            }  
            $zip->close(); 
            if($res == true){
               @unlink($dir."/".$this->file['name']);
            }
    ...
    protected function act_uploadbf() {
        if ($this->config['readonly'] || !isset($this->post['dir']))
            $this->errorMsg("Unknown error.");
    ...
    protected function getFilesbf($dir) {
        $thumbDir = "{$this->config['uploadDir']}/{$this->config['thumbsDir']}/$dir";
        $dir = "{$this->config['uploadDir']}/$dir";
        $return = array();
        $files = dir::content($dir, array('types' => "file"));
    ...
    protected function getTree($dir, $index=0) {
        ...
        //lyw 2015-01-15 修改目录权限
        if($index==1){
            foreach($dirs as $dk=>$dv){
                if($this->session['typejobs']&&$this->session['typejobs']!=$dv['name']){
                    unset($dirs[$dk]);
                }                
            }
        }
