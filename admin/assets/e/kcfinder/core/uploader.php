...
class uploader {
    ...
    public function __construct() {
        ...
        if ($this->config['uploadURL'] == "/") {
            ...
            //var_dump($this->config['uploadURL']);die;
            $this->config['uploadDir'] = strlen($this->config['uploadDir'])
                ? path::normalize($this->config['uploadDir'])
        ...
        if ($config['disabled'] || $config['readonly']) {
            ...
            if (isset($this->get['dir']) &&
                ...
                if (substr($udir, 0, strlen($dir)) !== $dir)
                    $message = $this->label("rrrrUnknown error.");
                else {
                    $l = strlen($dir);
                    $dir = "$udir/";
                    $udir = substr($udir, $l);
    ...
    protected function checkUploadedFile() {
        ...
        if(!preg_match("/^[A-Za-z\d_-]*$/i",substr($file['name'],0,strpos($file['name'],'.'))))
             return $this->label("文件名只能包含字母,数字,下划线,横杠,谢谢！");   
