browser.initFiles = function() {
    ...
    $.each(this.shows, function(i, val) {
     ...
     $("#files").bind("scroll",function() {  
                  $("#files div .thumb").each(function(){
                                if($(this).css('background-image')!='none') 
                                    return;
                                var fold = $('#files').height() + $('#files').scrollTop();
                                if( fold >= $(this).offset().top){
                                   $(this).css('background-image',$(this).attr('o'))
                                }
                            });
                 });
    this.statusDir();
    ...
    setTimeout(function() {
        ...
        $.each(browser.files, function(i, file) {
          ...
          // if(true){
               ...
               html += '<div class="file">' +
                    '<div class="thumb" o="url(\'' + icon + '\')" style="background-image:" ></div>' +
...
browser.returnFile = function(file) {
    var cek = file[0]['baseURI'].split('&');
    if(cek[1]=='CKEditor=content'){
        var fileURL = file.substr
            ? file : browser.uploadURL + '/' + browser.dir + '/' + file.data('name');
        fileURL = 'http://www.easyframework.com'+fileURL; 
        //console.log(fileURL);return false;
...
browser.returnFiles = function(files) {
    if (this.opener.callBackMultiple && files.length) {
     ...
     if (this.opener.callBackMultipleEditor && files.length) {
        var rfiles = [];
        $.each(files, function(i, file) {
            rfiles[i] = browser.uploadURL + '/' + browser.dir + '/' + $(file).data('name');
            rfiles[i] = _.escapeDirs(rfiles[i]);
        });
        this.opener.callBackMultipleEditor(rfiles);
        if (window.opener) window.close()
    }
};
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        if (this.opener.callBackMultiple||this.opener.callBackMultipleEditor) {
        ...
        file.addClass('selected');
        $('#fileinfo').html(data.name + ' (' + this.humanSize(data.size) + ', ' + data.date + ')');
        if (this.opener.callBack || this.opener.callBackMultiple||this.opener.callBackMultipleEditor) {
            html += '<a href="kcact:pick">' + this.label("Select") + '</a>';
            if (data.thumb) html +=
