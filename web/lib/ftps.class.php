...
class ftps {
    ...
    public function filelist($path = '') {
        ...
        if ($list = @ftp_nlist($this->link, $path)) {
            return $list;
        } else {
            $this->err_code = 11;
            return false;
     ...
     * 断线续传 上传文件
     * @param string $remote 远程存放地址
     * @param string $localhandler 本地存放地址
     */
    public function nb_put($remote, $local) {
        if (!$this->link) {
            $this->err_code = 2;
            return false;
        }
...
//		echo ftp_size($this->link, $remote);die;
        // 开始 ftp_nb_put 此函数上传文件的时候采用的是异步传输模式，也就意味着在文件传送的过程中，你的程序可以继续干其它的事情。, ftp_size($this->link, $remote)
        $ret = ftp_nb_put($this->link, $remote, $local, $this->mode);
     ...
     * 获取文件大小
     */
    public function getSize($remote) {
        if (!$this->link) {
            $this->err_code = 2;
            return false;
        }
        if ($fsize = @ftp_size($this->link, $remote)) {
            return $fsize;
        } else {
            $this->err_code = 12;
            return false;
    ...
    public function get_error() {
        ...
        $err_msg = array(
            ...
            '8' => 'Can not login into server',
            '9' => 'An error occurred in the upload file...',
            '10' => 'Chmod fail',
            '11' => 'Get files fail',
            '12' => 'Get file size fail'
        );
     ...
     * @return array 文件列表数据
     */
    public function rawlist($dirname) {
        if (!$this->link) {
            $this->err_code = 2;
            return false;
        }
        if ($list = @ftp_rawlist($this->link, $dirname)) {
            return $list;
        } else {
            $this->err_code = 5;
            return false;
    ...
    public function ftp_get($local,$remote){
        if (!$this->link) {
            $this->err_code = 2;
            return false;
        }
        return @ftp_get($this->link,$local,$remote,$this->mode);
    }
