...
/*
	...
	add by zrj
*/
function uc_user_register2($phone, $password, $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'register2', array('phone'=>$phone, 'password'=>$password, 'regip' => $regip));
}
/*
	手机号登录
	add by zrj
*/
function uc_user_login2($phone, $password) {
	$return = call_user_func(UC_API_FUNC, 'user', 'login2', array('phone'=>$phone,'password'=>$password));
...
/*
	...
	//2015-05-08 zpf plus
*/
function uc_user_login3($email, $password) {
	$return = call_user_func(UC_API_FUNC, 'user', 'login3', array('email'=>$email,'password'=>$password));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
function uc_login_record($username, $lastloginip, $lastlogintime, $flag){
	$return = call_user_func(UC_API_FUNC, 'user', 'user_login_record', array('username'=>$username,'lastloginip'=>$lastloginip,'lastlogintime'=>$lastlogintime,'flag'=>$flag));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
/* 
	...
	//改密码
*/
function uc_user_edit1($username, $oldpw, $newpw, $email, $phone, $ignoreoldpw = 0, $questionid = '', $answer = '', $flag = 0) {
	return call_user_func(UC_API_FUNC, 'user', 'edit1', array('username'=>$username, 'oldpw'=>$oldpw, 'newpw'=>$newpw, 'email'=>$email, 'phone'=>$phone, 'ignoreoldpw'=>$ignoreoldpw, 'questionid'=>$questionid, 'answer'=>$answer, 'flag'=>$flag));
}
/* 
	//2015-5-11 14:20:46 zpf plus
	//改信息
*/
//function uc_user_edit_info($uid, $nickname, $realname, $personal, $sex, $birthday = 0, $province = '', $answer = '', $flag = 0) {
function uc_user_edit_info($username, $user_data, $flag = 3) {
	return call_user_func(UC_API_FUNC, 'user', 'edit_info', array('username'=>$username, 'user_data'=>$user_data, 'flag'=>$flag));
}
...
//2015-5-9 13:45:57 zpf plus
function uc_get_user1($user_e_p) {
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user1', $user_e_p);
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
