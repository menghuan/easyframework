...
class usermodel {
	...
	*/
	function get_user_by_phone($phone){
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE phone='$phone'");
		return $arr;
	}
	/*
		添加用户
		add by zrj
	*/
	function add_user2($phone, $password, $uid = 0, $regip = '')
	{		
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd phone='$phone', password='$password', regip='$regip', regdate='".$this->base->time."', salt='$salt'");
	...
	/* 
		//2015-5-9 14:20:46 zpf plus
		$flag int 0:username 获得用户信息;1:email 获得用户信息;2:phone 获得用户信息;3:uid 获得用户信息; 
	*/
	function edit_user1($username, $oldpw, $newpw, $email, $phone, $ignoreoldpw = 0, $questionid = '', $answer = '', $flag) {
		if($flag == 0){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE username='$username'");	
			$keys_field = 'username';
		}elseif($flag == 1){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE email='$username'");	
			$keys_field = 'email';
		}elseif($flag == 2){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE phone='$username'");
			$keys_field = 'phone';
		}elseif($flag == 3){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE uid='$username'");
			$keys_field = 'uid';
		}
		if(!$ignoreoldpw && $oldpw && $data['password'] != md5(md5($oldpw).$data['salt'])) {
			return -1;
		}

		$sqladd = $newpw ? "password='".md5(md5($newpw).$data['salt'])."'" : '';
		$sqladd .= $email ? ($sqladd ? ',' : '')." email='$email'" : '';
		$sqladd .= $phone ? ($sqladd ? ',' : '')." phone='$phone'" : '';
		if($questionid !== '') {
			if($questionid > 0) {
				$sqladd .= ($sqladd ? ',' : '')." secques='".$this->quescrypt($questionid, $answer)."'";
			} else {
		...
		if($sqladd || $emailadd) {
			$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET $sqladd WHERE $keys_field='$username'");
			return $this->db->affected_rows();
		} else {
			return -7;
		}
	...
	/* 
		//2015-5-11 14:20:46 zpf plus
		$flag int 0:username 获得用户信息;1:email 获得用户信息;2:phone 获得用户信息;3:uid 获得用户信息; 
	*/
	function edit_user_info($username, $user_data, $flag) {
		if($flag == 0){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE username='$username'");	
			$keys_field = 'username';
		}elseif($flag == 1){
		...
		// $sqladd = $nickname ? "nickname='".$nickname."'" : '';
		// $sqladd .= $realname ? ($sqladd ? ',' : '')." realname=`$realname`" : '';
		// $sqladd .= $personal ? ($sqladd ? ',' : '')." personal=`$personal`" : '';
		// $sqladd .= $sex ? ($sqladd ? ',' : '')." sex=`$sex`" : '';
		// $sqladd .= $birthday ? ($sqladd ? ',' : '')." birthday=`$birthday`" : '';
		// $sqladd .= $province ? ($sqladd ? ',' : '')." province=`$province`" : '';
		// $sqladd .= $city ? ($sqladd ? ',' : '')." city=`$city`" : '';
		// $sqladd .= $isjob ? ($sqladd ? ',' : '')." isjob=`$isjob`" : '';
		// $sqladd .= $school ? ($sqladd ? ',' : '')." school=`$school`" : '';
		// $sqladd .= $typejob ? ($sqladd ? ',' : '')." typejob=`$typejob`" : '';
		
		$sqladd = "nickname='".$nickname."'";
		$sqladd .= ($sqladd ? ',' : '')." realname='$realname'";
		$sqladd .= ($sqladd ? ',' : '')." personal='$personal'";
		$sqladd .= ($sqladd ? ',' : '')." sex='$sex'";
		$sqladd .= ($sqladd ? ',' : '')." birthday='$birthday'";
		$sqladd .= ($sqladd ? ',' : '')." province='$province'";
		$sqladd .= ($sqladd ? ',' : '')." city='$city'";
		$sqladd .= ($sqladd ? ',' : '')." isjob='$isjob'";
		$sqladd .= ($sqladd ? ',' : '')." school='$school'";
		$sqladd .= ($sqladd ? ',' : '')." typejob='$typejob'";
		if($data['uid']){
	...
	/* 
		...
		$flag int 0:username 获得用户信息;1:email 获得用户信息;2:phone 获得用户信息;3:uid 获得用户信息; 
	*/
	function user_login_record($username, $lastloginip, $lastlogintime, $flag){
		if($flag == 0){
			$data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE username='$username'");	
			$keys_field = 'username';
		}elseif($flag == 1){
