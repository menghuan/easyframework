...
class usercontrol extends uc_base {
	...
	*/
	function onregister2() {
		$this->init_input();
		$phone = $this->input('phone');		//new add
		$password =  $this->input('password');
		$regip = $this->input('regip');
		if(($status = $this->_check_phone($phone)) <0){	
			$flag = $status;
		}else{
			$flag = $_ENV['user']->add_user2($phone, $password,0,$regip);			
		}	
		//$uid = $_ENV['user']->add_user2($phone, $password,0,$regip);
	...
	*/
	function onlogin2() {
		...
		$user = $_ENV['user']->get_user_by_phone($phone);
	...
	*/
	...
	/*
		登录记录
		//2015-5-11 16:52:20 zpf plus
	*/
	function onuser_login_record(){
		$this->init_input();
		$username = $this->input('username');
		$lastloginip = $this->input('lastloginip');
		$lastlogintime = $this->input('lastlogintime');
		$flag = $this->input('flag');
		$status = $_ENV['user']->user_login_record($username, $lastloginip, $lastlogintime, $flag);
		return $status;
	}
	...
	function onedit1() {
		...
		if(!$ignoreoldpw && $phone && ($status = $this->_check_phone($phone, $username)) < 0) {
			return $status;
		}
		
		$status = $_ENV['user']->edit_user1($username, $oldpw, $newpw, $email, $phone, $ignoreoldpw, $questionid, $answer,$flag);
	...
	//2015-5-9 16:56:49 zpf plus 
	function onedit_info() {
		$this->init_input();
		$username = $this->input('username');
		$user_data = $this->input('user_data');
		$flag = $this->input('flag');
		$status = $_ENV['user']->edit_user_info($username, $user_data, $flag);
	...
	//2015-5-7 16:56:49 zpf plus 
	function oncheck_phone() {
		$this->init_input();
		$phone = $this->input('phone');
		return $this->_check_phone($phone);
	} 
	...
	//2015-5-9 13:49:07 zpf plus
	function onget_user1() {
		$this->init_input();
		if($email = $this->input('email')) {
			$status = $_ENV['user']->get_user_by_email($email);
		}elseif($phone = $this->input('phone')){
			$status = $_ENV['user']->get_user_by_phone($phone);
		}
		if($status) {
			return $status;
		} else {
	...
	//2015-5-7 16:56:49 zpf plus 
	function _check_phone($phone, $username = '') {
